...
import com.wine.to.up.simple.parser.service.logging.SimpleParserNotableEvents;
...
public class KafkaController {
    ...
    /**
     * Sends messages into the topic "test". In fact now this service listen to that
     * topic too. That means that it causes sending and reading messages
     */
    @PostMapping(value = "/send")
    public void sendMessage(@RequestBody String message) {
        eventLogger.info(SimpleParserNotableEvents.I_CONTROLLER_RECEIVED_MESSAGE, message);
    ...
    public void sendMessageWithHeaders(@RequestBody DemoServiceMessage message) {
        ...
        eventLogger.warn(SimpleParserNotableEvents.W_SOME_WARN_EVENT, "Demo warning message");
        ...
        int sent = Stream.iterate(1, v -> v + 1).limit(3).map(n -> executorService.submit(() -> {
            int numOfMessages = 10;
            for (int j = 0; j < numOfMessages; j++) {
                kafkaSendMessageService.sendMessage(event);
                counter.incrementAndGet();
                eventLogger.info(SimpleParserNotableEvents.I_KAFKA_SEND_MESSAGE_SUCCESS, message);
            }
